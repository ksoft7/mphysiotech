...
function BlogTable({ blog }) {
  ...
  const handleDelete = () => {
    dispatch(deleteBlogPost(blog._id))
      .then(() => {
        toast.success("Product has been deleted.");
      })
      .catch((error) => {
        toast.error(
          error.response?.data?.message || "Failed to delete the blogPost."
        );
      });
  ...
  const handleEdit = () => {
    navigate(`/edit-blogPost/${blog._id}`);
  };
  return (
    <div style={styles.card}>
      <img
        ...
        style={styles.image}
      />
      <h3 style={styles.name}>{blog.title}</h3>
      <p style={styles.price}>{blog.contentTwo}</p>
